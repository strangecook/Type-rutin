...
import {
  ApproachContainer,
  ChartDiv,
} from "../../Styled/HomeStyle/ApproachStyled";
...
function Approach() {
  ...
  useEffect(() => {
    ...
    const fristData = `${
      ((ApproachTodayData.length + ApproachEveryData.length) /
        (filteredTodayData.length + EveryData.length)) *
      100
    }%`;
    ...
    const SecondData = `${
      (ApproachSevenDayDate.length / filteredSevenDayDate.length) * 100
    }%`;
    ...
    const thirdDate = `${
      (ApproachEveryDayDate.length / todayData.length) * 100
    }%`;
